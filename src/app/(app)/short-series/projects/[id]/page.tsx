...
export default function ProjectDetailPage() {
  ...
  const handleAddEpisode = async () => {
    ...
    try {
      ...
      setEpTitle('');
      toast(`已添加第 ${n} 集`);
      setTimeout(() => router.push(`/short-series/projects/${projectId}/episodes/${n}`), 500);
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="pd-topinfo">
        ...
        <span style={{ flex: 1 }} />
        <button className="back-link" onClick={() => router.push('/short-series/projects')}>
          &larr; 返回工作台
        </button>
      </div>
          ...
          {episodesLoading ? (
            ...
            episodes.map((ep) => (
              <div
                ...
                className="ep-row"
                onClick={() => router.push(`/short-series/projects/${projectId}/episodes/${ep.episodeNumber}`)}
              >
